...
class FileHandler(tornado.web.RequestHandler):
    ...
    def initialize(self, bak_path = None, path = None, auth = None):
        if not path :
            ...
            self.open_flag = True
            self.file_dir = os.path.join(path, "file")
            
        if bak_path:
            self.bak_path = os.path.join(bak_path, "file")
        else:
            self.bak_path = None
        ...
        self.return_url_prefix = "http://f" + str(settings.SHARD_ID)
        pass
    
    def get(self, group='common'):
        self.write("upload_file: " + group)
        pass
                        ...
                        try: 
                            ...
                            if end in ['txt', 'html', 'htm'] :
                                ...
                                if settings.ENV == 'product':
                                    ret["message"].append(self.return_url_prefix  + settings.DOMAIN  + "/file/" + group + "/" + rename)
                                elif settings.ENV == 'idc_test':
                                    ret["message"].append('http://' + settings.IDC_TEST_IP + "/file/" + group + "/" + rename)
                                elif settings.ENV == 'test':
                                    ret["message"].append('http://' + settings.TEST_IP + "/file/" + group + "/" + rename)
                                else:
