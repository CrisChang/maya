...
class ImageHandler(AbstractImageHandler):
    ...
    def write_file(self, file_dir, group, first_d, second_d, third_d, content, name, end):
        ...
        #im = Image.open(dst.decode('utf-8').encode(sys.getfilesystemencoding()))
        #(xx, yy) = im.size
        #print xx, yy
        #dst = os.path.join(new_d, name + '_' + xx + 'x' + yy +'.' + end)
        #im.save(dst.decode('utf-8').encode(sys.getfilesystemencoding()))
