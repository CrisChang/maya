...
class VideoHandler(tornado.web.RequestHandler):
    def initialize(self, bak_path = None, path = None, auth = None):
        if not path :
            ...
            self.open_flag = True
            self.file_dir = os.path.join(path, "video")
            
        if bak_path:
            self.bak_path = os.path.join(bak_path, "video")
        else:
            self.bak_path = None
        ...
        self.return_url_prefix = "http://v" + str(settings.SHARD_ID)
        pass
    
    def get(self, group='common'):
        self.write("upload_video: " + group)
        pass
                ...
                if self.request.files:
                    for f in self.request.files["files"]:
                        try: 
                            ...
                            if True or end in ['txt', 'html', 'htm'] :
                                m = hashlib.md5()
                                ...
                                if settings.ENV == 'product':
                                    ret["message"].append(self.return_url_prefix  + settings.DOMAIN  + "/video/" + group + "/" + rename)
                                elif settings.ENV == 'idc_test':
                                    ret["message"].append('http://' + settings.IDC_TEST_IP + "/video/" + group + "/" + rename)
                                elif settings.ENV == 'test':
                                    ret["message"].append('http://' + settings.TEST_IP + "/video/" + group + "/" + rename)
                                else:
